...
_PyEval_SetProfile(PyThreadState *tstate, Py_tracefunc func, PyObject *arg)
{
    ...
    _PyEval_StartTheWorld(interp);
    Py_XDECREF_HEAP(old_profileobj);  // pass tracing // needs to be decref'd outside of stop-the-world
    return ret;
}
...
_PyEval_SetProfileAllThreads(PyInterpreterState *interp, Py_tracefunc func, PyObject *arg)
{
    ...
    int ret = set_monitoring_profile_events(interp);
    _PyEval_StartTheWorld(interp);
    Py_XDECREF_HEAP(old_profileobjs);  // pass tracing // needs to be decref'd outside of stop-the-world
    return ret;
}
...
_PyEval_SetTrace(PyThreadState *tstate, Py_tracefunc func, PyObject *arg)
{
    ...
    _PyEval_StartTheWorld(interp);
    Py_XDECREF_HEAP(old_traceobj);  // pass tracing // needs to be decref'd outside stop-the-world
    return err;
}
...
_PyEval_SetTraceAllThreads(PyInterpreterState *interp, Py_tracefunc func, PyObject *arg)
{
    ...
    if (interp->sys_tracing_threads) {
        _Py_FOR_EACH_TSTATE_UNLOCKED(interp, tstate) {
            ...
            if (err != 0) {
                HEAD_UNLOCK(&_PyRuntime);
                _PyEval_StartTheWorld(interp);
                Py_XDECREF_HEAP(old_trace_objs); // pass tracing 
                return -1;
            }
        }
    }
    ...
    _PyEval_StartTheWorld(interp);
    Py_XDECREF_HEAP(old_trace_objs); // pass tracing  // needs to be decref'd outside of stop-the-world
    return err;
}
