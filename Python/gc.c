...
#ifndef ENABLE_RTGC_REF32
#  define ob_flags            ob_refcnt_split[PY_BIG_ENDIAN ^ 1]
#endif

#ifdef ENABLE_RTGC_LOG
#  define rtgc_log(cond, ...) if (cond) printf(__VA_ARGS__);
#else
...
static int circuit_cnt = 0;
const static bool fast_dealloc = false;

typedef struct _RTGC_Obj {
    PyGC_Head   head;
    PyObject    obj;
} RTGC_Obj;
...
static inline bool
rtgc_is_cyclic_root(PyObject* op) {
    return (op->ob_flags & RTGC_CIRCUIT_ROOT) != 0;
}


static inline void
rtgc_mark_cyclic_root(PyObject* op) {
    assert(!rtgc_is_cyclic(op));
    op->ob_flags |= RTGC_CIRCUIT_ROOT;
}


static inline void
rtgc_unmark_cyclic(PyObject* op) {
    op->ob_flags &= ~(RTGC_CIRCUIT | RTGC_CIRCUIT_ROOT);
}
...
{
    assert(refs < (INT_MAX >> _PyGC_PREV_SHIFT));
...
#ifdef ENABLE_RTGC_GC
    for (size_t i = 0; i < sizeof(gcstate->cyclic) / sizeof(gcstate->cyclic[0]); i ++) {
        PyListObject* op = &gcstate->cyclic[i].obj;
        ((PyObject*)op)->ob_refcnt = 0xFFFF;
...
validate_list(PyGC_Head *head, enum flagstates flags)
{
    ...
    while (gc != head) {
        ...
        if (trueprev != prev) {
            printf("prev: %p(true: %p), gc: %p(as next of prev: %p)\n", prev, trueprev, gc, GC_NEXT(prev));
...
visit_decref(PyObject *op, void *parent)
{
    ...
    if (_PyObject_IS_GC(op)) {
        ...
        if (gc_is_collecting(gc)) {
            // rtgc_log(rtgc_is_cyclic(op), "revist circuit %p\n", op);
            gc_decref(gc);
        }
    }
    return 0;
...
{
    PyObject *circuit_root = param;

    // printf("op: %p, rc: %d, cyclic: %d\n", op, op->ob_refcnt, rtgc_is_cyclic(op));
    if (_PyObject_IS_GC(op)) {
        if (op == circuit_root) {
            PyGC_Head *gc = AS_GC(op);
            gc->_gc_prev |= _PyGC_PREV_MASK_COLLECTING;
        }
        else if (rtgc_is_linear_cyclic(op)) {
            traverseproc traverse = Py_TYPE(op)->tp_traverse;
            (void) traverse(op,
                            visit_decref_circuit,
                            circuit_root);
            if (gc_is_collecting(AS_GC(circuit_root))) {
                PyGC_Head *gc = AS_GC(op);
                PyGC_Head* last = GC_PREV(rtStack.unreachable);                
                gc->_gc_prev |= _PyGC_PREV_MASK_COLLECTING;
                rtgc_unmark_cyclic(op);
                gc_list_move(gc, rtStack.unreachable);
                last->_gc_next |= NEXT_MASK_UNREACHABLE;
...
visit_reachable(PyObject *op, void *arg)
{
    ...
    if (! gc_is_collecting(gc)) {
#ifdef ENABLE_RTGC_GC
        if ((int)op->ob_refcnt == 2) {
            RTGC_Obj* pgc = parent;
            for (int i = 0; i < MAX_CIRCLE_LEN && (int)pgc->obj.ob_refcnt == 1; i ++) {
                pgc = (RTGC_Obj*)GC_PREV(&pgc->head);
                if ((intptr_t)pgc <= 8) break;
                // rtgc_log(true, "scan circuit: %p parent: %p\n", pgc, parent);
                // if (rtgc_is_cyclic(&pgc->obj)) {
                //     rtgc_log(true, "circuit???: %p (len: %d)\n", gc, i);
                // }
                if (pgc == (void*)gc) {
                    rtgc_log(true, "circuit root found: %p (len: %d)\n", gc, i);
                    for (pgc = parent; pgc != gc; pgc = (RTGC_Obj*)GC_PREV(&pgc->head)) {
                        rtgc_mark_cyclic(&pgc->obj);
                    }
                    rtgc_mark_cyclic_root(&((RTGC_Obj*)gc)->obj);
                    break;
                }
            }
        }
    ...
    if (gc->_gc_next & NEXT_MASK_UNREACHABLE) {
        ...
        rtgc_log((prev->_gc_next & NEXT_MASK_UNREACHABLE) == 0
                || (next->_gc_next & NEXT_MASK_UNREACHABLE) == 0,
            "gc: %p, prev: %p, next: %p\n", gc, prev, next);

        rtgc_log(rtgc_is_cyclic(op),
            "resurrect_circuit: %p, prev: %p, next: %p\n", gc, prev, next);
        ...
        if ((int)op->ob_refcnt == 1) {
            _PyGCHead_SET_PREV(gc, &parent->head);
        } else
#endif
        gc_set_refs(gc, 1);
    }
 ...
 */
static void
move_unreachable(PyGC_Head *young, PyGC_Head *unreachable, GCState* gcstate)
{
#ifdef ENABLE_RTGC_GC        
...
#ifdef ENABLE_RTGC_GC    
    prev = young;
    PyGC_Head* c0_prev = rtStack.circuit_roots;
    PyGC_Head* c0_next = GC_NEXT(rtStack.circuit_roots);
    for (gc = GC_NEXT(young); gc != young; gc = (PyGC_Head*)prev->_gc_next) {
        // rtgc_log(true, "prev: %p, c0_prev: %p, gc: %p, next: %p\n", prev, c0_prev, gc, GC_NEXT(gc));
        int flags = ((RTGC_Obj*)gc)->obj.ob_flags;
        if (flags & (RTGC_CIRCUIT | RTGC_CIRCUIT_ROOT)) {
            prev->_gc_next = gc->_gc_next;
            if (flags & (RTGC_CIRCUIT_ROOT)) {
                c0_prev->_gc_next = (intptr_t)gc;
                _PyGCHead_SET_PREV(gc, c0_prev);
                c0_prev = gc;
            } else {
                PyGC_Head* cp = GC_PREV(gc);
                if (c0_prev == cp) {
                    c0_prev = gc;
                } else {
                    gc->_gc_next = cp->_gc_next;
                    _PyGCHead_SET_PREV((PyGC_Head*)gc->_gc_next, gc);
                }
                cp->_gc_next = (intptr_t)gc;
            }
        } else {
            _PyGCHead_SET_PREV(gc, prev);
            prev = gc;
    ...
    _PyGCHead_SET_NEXT(c0_prev, c0_next);
    _PyGCHead_SET_PREV(c0_next, c0_prev);
    _PyGCHead_SET_PREV(young, prev);
#endif
}
...
static inline void
deduce_unreachable(PyGC_Head *base, PyGC_Head *unreachable, GCState* gcstate) {
...
#endif
    move_unreachable(base, unreachable, gcstate);  // gc_prev is pointer again
    validate_list(base, collecting_clear_unreachable_clear);
...
gc_collect_main(PyThreadState *tstate, int generation, _PyGC_Reason reason)
{
...
#ifdef ENABLE_RTGC_GC
    rtStack.circuit_roots = GEN_HEAD(gcstate, NUM_GENERATIONS-1);
    rtStack.unreachable = &unreachable;
    rtStack.generation = generation;
    gc_list_init(&unreachable);
#endif
    ...
    if (generation == NUM_GENERATIONS-1) {
        int cnt_cyclic = 0;
        for (PyGC_Head* gc = GC_NEXT(rtStack.circuit_roots); gc != &gcstate->cyclic[1].head; gc = GC_NEXT(gc)) {
            rtgc_unmark_cyclic(FROM_GC(gc));
            cnt_cyclic ++;
        }
        rtgc_log(true, "unmarked cyclic %d\n", cnt_cyclic);
        finalize_unlink_gc_head(&gcstate->cyclic[0].head);
        finalize_unlink_gc_head(&gcstate->cyclic[1].head);

        PyGC_Head* first = GC_NEXT(rtStack.circuit_roots);
        _PyGCHead_SET_NEXT(&gcstate->cyclic[0].head, &gcstate->cyclic[1].head);
        _PyGCHead_SET_PREV(&gcstate->cyclic[1].head, &gcstate->cyclic[0].head);
        _PyGCHead_SET_NEXT(&gcstate->cyclic[1].head, first);
        _PyGCHead_SET_PREV(&gcstate->cyclic[0].head, rtStack.circuit_roots);
        ...
        for (PyGC_Head* gc = GC_NEXT(rtStack.circuit_roots); gc != &gcstate->cyclic[0].head; gc = gc_next) {
            ...
            gc_next = GC_NEXT(gc);
            PyObject *op = FROM_GC(gc);
            if (rtgc_is_cyclic_root(op) && (int)op->ob_refcnt == 1) {
                gc_list_move(gc, young);
                while (true) {
                    gc = gc_next;
                    op = FROM_GC(gc);
                    if (!rtgc_is_cyclic(op)) break;
                    rtgc_unmark_cyclic(op);
                    gc_next = GC_NEXT(gc);
                    gc_list_move(gc, young);
                }
...
_PyGC_Freeze(PyInterpreterState *interp)
{
    ...
    PyGC_Head head;
    gc_list_init(&head);
    for (int i = NUM_GENERATIONS; --i >= 0; ) {
        gc_list_merge(GEN_HEAD(gcstate, i), &head);
        gcstate->generations[i].count = 0;
    }
    gc_list_merge(&gcstate->permanent_generation.head, &head);
    gc_list_merge(&head, &gcstate->permanent_generation.head);
#else
    for (int i = 0; i < NUM_GENERATIONS; ++i) {
