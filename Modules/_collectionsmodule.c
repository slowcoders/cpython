...
deque_append_lock_held(dequeobject *deque, PyObject *item, Py_ssize_t maxlen)
{
    ...
    if (NEEDS_TRIM(deque, maxlen)) {
        ...
        Py_DECREF(olditem); // deque_popleft_impl 의 반환값을 deref
    } else {
        deque->state++;
    }
    return 0;
                           ...
                           Py_ssize_t maxlen)
{
    ...
    if (NEEDS_TRIM(deque, maxlen)) {
        ...
        Py_DECREF(olditem); // deque_pop_impl 의 반환값을 deref
    } else {
        deque->state++;
    }
    return 0;
...
defdict_init(PyObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (newargs == NULL)
        return -1;
    dd->default_factory = Py_XNewRef_HEAP(newdefault);
    result = PyDict_Type.tp_init(self, newargs, kwds);
    Py_DECREF(newargs);
    Py_XDECREF_HEAP(olddefault); // heap-ref
    return result;
}
